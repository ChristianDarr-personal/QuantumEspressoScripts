...
K_POINTS {crystal_b}
10
  0.000  0.000  0.000  300 !G
  0.500  0.000  0.500  300 !X
  0.500  0.250  0.750  300 !W
  0.375  0.375  0.750  300 !K
  0.000  0.000  0.000  300 !G
  0.500  0.500  0.500  300 !L
  0.625  0.250  0.625  300 !U
  0.500  0.250  0.750  300 !W
  0.500  0.500  0.500  300 !L
  0.375  0.375  0.750  300 !K
...
srun pw.x < FH_bands.in > FH_bands.out

# extract Etot from output
etot=`grep -e ! FH_bands.out | awk '{print $(NF-1)}'`
#mtot=`grep 'total magnetization' FH_scf.out | tail -1 | awk '{print $(NF-2)}'`
echo $k $etot >> Full.fcc
done
cat Full.fcc
